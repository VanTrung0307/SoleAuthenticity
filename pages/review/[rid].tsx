...
export default function ReadingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => { 
        try {
          const res = await fetch(`https://soleauthenticity.azurewebsites.net/api/reviews/review/${rid}`);
        const data = await res.json();
        console.log(data.data);

        setData(data.data);

        const resFull = await fetch('https://soleauthenticity.azurewebsites.net/api/reviews?page=1&pageSize=10');
  ...
  return (
    <>
      ...
      {data ? <div className="my-10 mx-auto" style={{ textAlign: "center" }}>
        <Image
          height="350"
          width="700"
          src={`${data?.avatar}`}
      ...
      <div className="container my-20 flex flex-col justify-center mx-auto">
        ...
        {datas?.filter((id) => data?.productId !== id.productId).map((item) => {
